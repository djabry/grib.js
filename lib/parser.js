...
const Grib2Message = function(sections) {
    let currentField = {}, sectionIdx, section, key;
...
const parseIndicator = function(binary) {
    const rv = {};
...
const dataRepresentationTemplateParsers = {
...
const gridParsers = {
    ...
    40: function(binary) {
        ...
        rv.scanningMode = binary.read('uint8');
        const basicAngle = ((rv.basicAngle === 0) || (rv.basicAngle === 0xffffffff)) ? 1 : rv.basicAngle;
        const scale = 1e-6 / basicAngle;
        rv.la1 *= scale; rv.lo1 *= scale;
        rv.la2 *= scale; rv.lo2 *= scale;
        rv.di *= scale;
        return rv;
...
const sectionParsers = {
    ...
    3: function(binary) {
        ...
        let gridDefnParser = gridParsers[rv.templateNumber];
        if(!gridDefnParser) {
...
const parseSection = function(binary) {
    let section = {}, startOffset = binary.tell(),
        sectionParseFunc;
...
const parseSections = function(binary) {
    let rv = [], section;
...
const parseScaledValue = function (binary) {
    let scale = binary.read('uint8'), value = binary.read('uint32');
    return value * Math.pow(10, -scale);
};

// Parser for basic angle
const parseBasicAngle = function (binary) {
    let basicAngle = binary.read('uint32'), basicAngleSub = binary.read('uint32');
